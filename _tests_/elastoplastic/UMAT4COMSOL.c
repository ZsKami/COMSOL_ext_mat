...
extern void umat_(double stress[6], double *statev, double ddsdde[6][6], // 3, accumulated is 3
  double sse, double spd, double scd, double rpl, double ddsddt[6], // 5, accumulated is 8
  double drplde[6], double drpldt, double stran[6], double dstran[6], // 4, accumulated is 12
  double time[2], double *dtime, double temp, double dtemp, double predef[1], // 5, accumulated is 17
  double dpred[1], char cmname, int *ndi, int *nshr, int *ntens, int *nstatv, // 6, accumulated is 23
  double *props, int *nprops, double coords, double drot, double *pnewdt, // 5, accumulated is 28
  double celent, double dfgrd0[3][3], double dfgrd1[3][3], int noel, int npt, // 5, accumulated is 33
  int layer, int kspt, int kstep, int kinc); // 4, accumulated is 37
...
// umat
    umat_(&stress[0],&states1[0],ddsdde,ignoredd,ignoredd,ignoredd,ignoredd, // &ddsdde[0] -> ddsdde
